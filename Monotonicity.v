...
(** We first attempt to use any matching hypothesis. This takes
  ...
  be flipped in the hypothesis, compared to the goal. This is because 
  the relation may be of the form (or may need to be instantiated to)
  [flip _]. In that case [flip_relim] will allow us to use the
  property (and instantiate as required). This is especially common in
  the course of solving a goal of the form [(R --> R') f g], or goals
  generated by the setoid rewriting system which have the form
  [Proper (?R1 ==> ... ==> ?Rn ==> flip impl)], and where the [?Ri]s
  will generally need to be instantiated as [flip ?Ri']. We do not
  pursue a similar strategy when looking up candidates from the
  typeclass instance database, because we expect the user to normalize
  such properties before they declare them, for example declare an
  instance of [Related (R1 ++> R2 --> R3) g f] rather than an instance
  of [Related (R1 --> R2 ++> flip R3) f g].
